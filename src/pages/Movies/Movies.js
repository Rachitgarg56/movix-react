...
const Movies = () => {
  ...
  const getData = async () => {
    ...
    const dataArr = await fetchApi(`https://api.themoviedb.org/3/discover/movie?page=${page}`);
    // console.log(`call ${page}`, dataArr);
  ...
  const getGenreData = async (genreIdsArr) => {
    const page = (genre.length / 20) + 1;
    const genreIdsStr = genreIdsArr.join(',')
    const dataArr = await fetchApi(`https://api.themoviedb.org/3/discover/movie?page=${page}with_genres=${genreIdsStr}`);
    // console.log('genre',dataArr);
    setGenre([...genre, ...dataArr.results])
  }
      ...
      <div className="flex flex-col gap-8 px-4 py-20 pt-28">
        ...
        >
          ...
          <InfiniteScroll
            ...
            dataLength={movies.length || genre.length} 
            next={getData}
            hasMore={true}
